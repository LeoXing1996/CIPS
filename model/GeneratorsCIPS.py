...
from .blocks import (LFF, ConstantInput, EqualLinear, PixelNorm, StyledConv,
                     StyledResBlock, ToRGB)
...
class CIPSskip(nn.Module):
    def __init__(self,
        ...
        self.conv1 = StyledConv(int(multiplier * hidden_size),
                                in_channels,
                                1,
                                style_dim,
    ...
    def forward(
        ...
        if truncation < 1:
            latent = truncation_latent + truncation * (latent -
                                                       truncation_latent)
...
class CIPSres(nn.Module):
    def __init__(self,
        ...
        multiplier = 2
        self.linears.append(
            StyledConv(int(multiplier * hidden_size),
                       in_channels,
                       1,
                       style_dim,
                       demodulate=demodulate,
